...
public class ConvertContainerAnnotationsTests
{
    ...
    public void RoundTripsArbitraryAnnotationValues()
    {
        ...
        Assert.True(decoder.Execute());
        Assert.Equal(2, decoder.DecodedAnnotations.Length);
        for (int i = 0; i < annotations.Length; i++)
        {
            Assert.Equal(annotations[i].ItemSpec, decoder.DecodedAnnotations[i].ItemSpec);
            Assert.Equal(annotations[i].GetMetadata("Scope"), decoder.DecodedAnnotations[i].GetMetadata("Scope"));
            Assert.Equal(annotations[i].GetMetadata("Value"), decoder.DecodedAnnotations[i].GetMetadata("Value"));
        }
    }
